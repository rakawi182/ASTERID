...
Author: ASTERID Consortium — Jolotundo Observatory
Version: 4.0 (2026-07-10)
"""
...
def _find_nearest_hour_index(time_list: List[str], target_utc: datetime) -> int:
    ...
    for i, t_str in enumerate(time_list):
        # Hapus 'Z' jika ada, meskipun Open-Meteo mengembalikan format tanpa 'Z'
        clean_t_str = t_str.replace('Z', '')
        
        # Parse string menjadi objek datetime
        dt = datetime.fromisoformat(clean_t_str)
        
        # PERBAIKAN: Sematkan tzinfo UTC secara eksplisit agar .timestamp() tidak melenceng ke WIB
...
# -----------------------------------------------------------------------------
# CORE API REQUEST (extended parameters with safe handling)
# -----------------------------------------------------------------------------
def _fetch_from_openmeteo(lat: float, lon: float, start_date: str, end_date: str) -> Optional[Dict[str, Any]]:
    """
    Fetch data from Open-Meteo ECMWF API with retries.
    Uses extended parameters that are known to be supported.
    """
    # We request additional parameters; if they are missing, we handle it later.
    params = {
        "latitude": lat,
        "longitude": lon,
        "hourly": [
    ...
    for attempt in range(max_retries):
        try:
            req = urllib.request.Request(full_url, headers={'User-Agent': 'AsteridMeteo/4.0'})
...
def get_ecmwf_meteo(
    ...
    """
    Retrieve ECMWF IFS HRES data for a given location and target height.
    Uses caching (6-hour validity) and fallback to the most recent cache.
    """
    if utc_time is None:
    ...
    # ---- 1. Check cache for this exact hour ----
    ...
    if raw is not None:
        try:
            with open(cache_file, 'w') as f:
                json.dump(raw, f)
            # Clean up old caches (keep only 10 most recent)
            all_caches = sorted(
    ...
    # ---- 3. Offline fallback: use most recent valid cache ----
    all_caches = sorted(
        [f for f in os.listdir(cache_dir) if f.startswith("ecmwf_cache_") and f.endswith(".json")],
        key=lambda x: os.path.getmtime(os.path.join(cache_dir, x)),
        reverse=True
...
# -----------------------------------------------------------------------------
# PARSE AND EXTRAPOLATE (with optional fields)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# REPORT GENERATION (with optional fields)
# -----------------------------------------------------------------------------
def generate_meteo_report(lat: float, lon: float, height_m: float, cache_dir: Optional[str] = None) -> None:
    ...
    print(f"  {'Wind Direction (10 m)':<30}: {data['wind_direction_deg']:>7.0f} ° (True North)")
...
# -----------------------------------------------------------------------------
# CSV EXPORT (with optional fields)
# -----------------------------------------------------------------------------
def export_meteo_csv(lat: float, lon: float, height_m: float,
    ...
    fieldnames = [
        ...
        'precipitation_mm', 'cloud_cover_total_pct',
...
if __name__ == "__main__":
    JOLOTUNDO_LAT = -7.609444
    JOLOTUNDO_LON = 112.595556
    JOLOTUNDO_ORTHO = 554.509

    print("⏳ Initialising ECMWF IFS HRES retrieval engine...")
    try:
        generate_meteo_report(JOLOTUNDO_LAT, JOLOTUNDO_LON, JOLOTUNDO_ORTHO)
        # Optional: export CSV
        # export_meteo_csv(JOLOTUNDO_LAT, JOLOTUNDO_LON, JOLOTUNDO_ORTHO)
