...
ENGINE: VSOP87A (Earth) + ELP/MPP02 (Moon)
============================================================
...
IERS Conventions (2010).  The engine uses the VSOP87A theory
for the Earth and ELP/MPP02 for the Moon.

The computation follows the rigorous astrometric reduction
sequence from the external reference frame to the observer's
local horizon (Heliocentric → Geocentric → Topocentric → Observed).

All IERS 2010 geophysical corrections (solid Earth tides,
ocean tide loading, pole tide, atmospheric loading, non‑tidal
loading, vertical deflection DoV, plate motion) are applied in
full double‑precision without any simplification.

REFERENCES
----------
1. IAU SOFA Software Collection. http://www.iausofa.org
2. McCarthy, D.D. & Petit, G. (2010). IERS Conventions (2010).
3. Bretagnon P., Francou G. (1988). VSOP87 planetary theory.
4. Chapront, J., Chapront‑Touzé, M. & Francou, G. (1997). ELP/MPP02.
5. Vallado, D.A. (2013). Fundamentals of Astrodynamics.
6. Allen, C.W. (1976). Astrophysical Quantities.
7. Altamimi, Z. et al. (2023). ITRF2020‑PMM.

AUTHOR        : ASTERID Consortium – Jolotundo Research Observatory
VERSION       : 4.2 (2026-06-21) – Hierarchical report with GCRS cartesian vectors
"""
...
def generate_report(
    ...
    The reduction sequence rigorously follows the IAU SOFA/IERS 2010 hierarchy.
    """
    # ---- 1. TIME SCALES ----
    ...
    # ---- 5. EPHEMERIS ORCHESTRATOR (VSOP87A + ELP/MPP02) ----
    ephem = AstronomicalEphemeris(eop_file=eop_file)
    lat_rad = math.radians(lat_deg)
    lon_rad = math.radians(lon_deg)

    # ---- 6. HELIOCENTRIC FRAME: EARTH (VSOP87A) ----
    earth_pos_ecl, earth_vel_ecl = ephem.earth_heliocentric_ecliptic(tt_jd)  # AU, AU/day
    earth_pos_km = np.array(earth_pos_ecl) * AU2KM
    earth_vel_km_day = np.array(earth_vel_ecl) * AU2KM
    earth_dist_au = np.linalg.norm(earth_pos_ecl)
    earth_dist_km = np.linalg.norm(earth_pos_km)
    ...
    # Sun ecliptic longitude from Earth's heliocentric position
    sun_pos_ecl = -np.array(earth_pos_ecl)   # from Earth to Sun
    sun_lon = math.atan2(sun_pos_ecl[1], sun_pos_ecl[0])

    # Moon ecliptic longitude from GCRS vector rotated to ecliptic J2000
    ...
    return {
        ...
        # Heliocentric Earth (VSOP87A)
        "earth_pos_km": earth_pos_km,
        "earth_vel_km_day": earth_vel_km_day,
        "earth_dist_km": earth_dist_km,
        "earth_dist_au": earth_dist_au,
...
# -------------------------------------------------------------------------
# PROFESSIONAL REPORT PRINTER (Hierarchical SOFA/IAU Order, Mobile Layout)
# -------------------------------------------------------------------------
def print_report(data: Dict) -> None:
    """Generate a high-precision scientific report optimized for mobile screens."""
    ...
    print(" ** EPHEMERIS ENGINE : VSOP87A + ELP/MPP02 **".center(w))
    print(" (VSOP87A for Earth, ELP/MPP02 for Moon)".center(w))
    ...
    # [5] HELIOCENTRIC EARTH (VSOP87A)
    print("\n[5] HELIOCENTRIC EARTH STATE (VSOP87A)")
    print(sep)
    earth_pos = data['earth_pos_km']
    earth_vel = data['earth_vel_km_day']
    print(f"{'Position X':<{pad}}: {earth_pos[0]:>18.3f} km")
    print(f"{'Position Y':<{pad}}: {earth_pos[1]:>18.3f} km")
    print(f"{'Position Z':<{pad}}: {earth_pos[2]:>18.3f} km")
    print(f"{'Velocity Vx':<{pad}}: {earth_vel[0]:>18.6f} km/day")
    print(f"{'Velocity Vy':<{pad}}: {earth_vel[1]:>18.6f} km/day")
    print(f"{'Velocity Vz':<{pad}}: {earth_vel[2]:>18.6f} km/day")
    print(f"{'Distance from Sun (km)':<{pad}}: {data['earth_dist_km']:>18.3f} km")
    print(f"{'Distance from Sun (AU)':<{pad}}: {data['earth_dist_au']:>18.6f} AU")
    print(f"{'Reference Frame':<{pad}}: Dynamical equinox & ecliptic")
    print(f"{'':<{pad}}  J2000 (VSOP87A)")
    ...
    print(f"{'Ephemeris Sources':<{pad}}: VSOP87A (Earth),")
...
if __name__ == "__main__":
    print("⏳ Computing comprehensive ephemeris + plate kinematics (VSOP87A)...")
    try:
