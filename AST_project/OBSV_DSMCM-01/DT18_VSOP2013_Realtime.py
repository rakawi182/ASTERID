...
def generate_report(
    ...
    # ---- 8. TOPOCENTRIC FRAME: CIRS & Equinox ----
...
def print_report(data: Dict) -> None:
    ...
    # [2] EARTH ORIENTATION
    section_header("[2] EARTH ORIENTATION PARAMETERS (IERS Bulletin A)")
    ...
    # [11] PHOTOMETRY
    section_header("[11] PHOTOMETRY & APPARENT MAGNITUDE")
    print(f"  {'Sun (V band)':<30}: {data['sun_apparent_mag']:.2f} mag (IAU standard)")
    print(f"  {'Moon (V band)':<30}: {data['moon_apparent_mag']:.2f} mag (Allen 1976)")

    # [12] LUNAR LIBRATION
    section_header("[12] LUNAR LIBRATION (LLIB04)")
    ...
    # [13] PLATE MOTION
    section_header("[13] KINEMATIC PLATE MOTION (ITRF2020‑PMM)")
    ...
    # [14] OBSERVER NOTES
    section_header("[14] OBSERVER & REDUCTION NOTES")
    print(f"  {'Geodetic Latitude':<30}: {OBSERVER_LAT_DEG:+.6f}°")
    print(f"  {'Geodetic Longitude':<30}: {OBSERVER_LON_DEG:+.6f}°")
    print(f"  {'Orthometric Height':<30}: {OBSERVER_HEIGHT_M:.3f} m (EGM2008)")
