...
# -*- coding: utf-8 -*-
"""
SM_IPS2000emb.py – High‑Precision Astronomical Ephemeris Orchestrator (IPS2000)
================================================================
Menggabungkan semua modul inti (IPS2000, ELP/MPP02, EarthRotation,
StationDisplacement, Atmospheric_refraction, Site_Geophysic) untuk
menghasilkan ephemeris toposentrik presisi tinggi.

Fitur:
    - Posisi Matahari & Bulan (GCRS, CIRS, ITRS, Horizontal)
    - Semua koreksi IERS 2010 (CIP‑CIO, nutasi, presesi, EOP, FCN)
    - Light‑time relativistik (Shapiro delay)
    - Aberasi dan defleksi gravitasi
    - Refraksi atmosfer (GPT3 + VMF3 + APG)
    - Deformasi kerak (solid tide, ocean tide, pole tide, loading)
    - Defleksi Vertikal (DoV) dinamis dari grid EGM2008 lokal
    - Koreksi ketinggian elipsoidal untuk paralaks
    - Kinematika lempeng tektonik (ITRF2020‑PMM)
"""
...
# ============================================================================
# ORCHESTRATOR CLASS (Fully IERS 2010 Compliant + DoV & Ellipsoidal Height)
# ============================================================================
class AstronomicalEphemeris:
    def __init__(self, eop_file: str = "EOP_20u24_C04_one_file_1962-now.txt",
        ...
        # Hapus inisialisasi VSOP87A
        # self.vsop_earth = VSOP87A(VSOP87A.find_file("ear"))
        ...
        try:
            ...
            # Ambil konstanta statis dari Atmospheric_refraction
    ...
    # -----------------------------------------------------------------------
    # Metode Ephemeris Dasar
    # -----------------------------------------------------------------------
    def earth_heliocentric_ecliptic(self, tt_jd: float) -> Tuple[np.ndarray, np.ndarray]:
        """
        Menghitung posisi dan kecepatan heliosentrik Bumi (bukan EMB)
        menggunakan IPS2000 untuk EMB dan ELP/MPP02 untuk Bulan.
        Kembali dalam AU dan AU/hari.
        """
        tdb = self.tt_to_tdb(tt_jd)
        
        # 1. Posisi dan kecepatan EMB dari IPS2000 (satuan AU dan AU/hari)
        emb_pos, emb_vel = emb_heliocentric_ecl(tdb)
        
        # 2. Posisi dan kecepatan Bulan dari ELP (km dan km/hari)
        tj_days = tdb - J2000_JD
        xyz_moon = ELP_MPP02_full.elpmpp02(tj_days, icor=1)
        
        moon_pos_au = np.array([xyz_moon[0], xyz_moon[1], xyz_moon[2]]) / AU2KM
        moon_vel_au = np.array([xyz_moon[3], xyz_moon[4], xyz_moon[5]]) / AU2KM
        
        # 3. Heliocentric Bumi = EMB - (rasio massa) * Bulan
        earth_pos = np.array(emb_pos) - self.mu * moon_pos_au
        earth_vel = np.array(emb_vel) - self.mu * moon_vel_au
    ...
    def sun_geocentric_gcrs(self, tt_jd: float, unit: bool = False) -> np.ndarray:
        ...
        if unit: return unit_vector(sun_pos_gcrs)
    ...
    def moon_geocentric_gcrs(self, tt_jd: float, unit: bool = False) -> np.ndarray:
        ...
        if unit: return unit_vector(pos_gcrs)
        return pos_gcrs
    ...
    def earth_velocity_gcrs(self, tt_jd: float) -> np.ndarray:
        # Ambil kecepatan Bumi langsung dari perhitungan IPS2000 + ELP di atas
        _, v_earth_ecl = self.earth_heliocentric_ecliptic(tt_jd)
        
        # Rotasi ke ekuator (FK5) lalu terapkan frame bias ke GCRS
        R = rot_x(-84381.406 * ARCSEC_TO_RAD)
    ...
    def sun_ecliptic_of_date(self, tt_jd: float, unit: bool = False,
        ...
        tt_jd : float
            Waktu dalam JD TT.
        unit : bool
            Jika True, posisi GCRS dalam unit vektor (tanpa jarak), jarak = 1.
            Jika False, posisi dalam km dan jarak dalam km (default).
        degrees : bool
            Jika True, bujur dan lintang dalam derajat; jika False, dalam radian.
        
        Returns
        -------
        lon, lat, r : float
            Bujur ekliptika, lintang ekliptika, dan jarak (dalam satuan sesuai unit).
        """
    ...
    def moon_ecliptic_of_date(self, tt_jd: float, unit: bool = False,
                              degrees: bool = False):
        """
        Hitung bujur, lintang, dan jarak Bulan dalam sistem ekliptika tanggal.
    ...
    def _ecliptic_of_date_from_gcrs(self, pos_gcrs, tt_jd, degrees=False):
        """
        Fungsi internal: ubah vektor GCRS ke ekliptika tanggal.
        Menggunakan matriks precession‑nutation tanpa bias
        (karena pos_gcrs sudah dalam GCRS).
        """
        ...
        # Matriks precession (P): GCRS (mean equator J2000) → mean equator of date
        P = rot_z(-pre['chiA']) @ rot_x(pre['omegaA']) @ rot_z(-pre['psiA']) @ rot_x(eps0)

        # Matriks nutation (N): mean equator of date → true equator of date
        N = rot_x(-pre['epsA']) @ rot_z(-dpsi) @ rot_x(pre['epsA'] + deps)

        # Gabungan tanpa bias: GCRS → true equator of date
        PN = N @ P
        pos_true_eq = PN @ pos_gcrs

        # Rotasi dari true equator ke true ecliptic ( terhadap sumbu X )
...
if __name__ == "__main__":
    ...
    print("Initializing High-Precision Orchestrator with IPS2000, DoV & Ellipsoidal Height...")
    ...
    print(" WITH IPS2000, DOPPLER (DoV) & ELLIPSOIDAL HEIGHT CORRECTIONS")
    print("=" * 80)
