...
# -*- coding: utf-8 -*-
"""
AstroCalc_VSOP2013.py – High-Precision Ephemeris (VSOP2013 Engine)
==================================================================
Fitur:
    ...
    - ** Ephemeris Matahari menggunakan VSOP2013 (Fienga et al. 2013) **
"""
...
from Coord_Transform import cartesian_to_spherical, rot_x, unit_vector

# -------------------------------------------------------------------------
# Ambil ELP_MPP02_full yang sudah diimpor oleh SM_VSOP2013 (hindari circular import)
# -------------------------------------------------------------------------
import sys
...
# ============================================================================
# 1. STELLAR ABERASI (copy dari SM_VSOP2013 agar tidak import error)
# ============================================================================
...
def compute_for_date(
    ...
    lod_result = lod_engine.get_lod_precision(jd_utc)
    lod_total_ms = lod_result['lod_total']
    # ------------------------------------
    ...
    # --- 4h. Apparent topocentric (az/alt, refraction) from SM_VSOP2013 ---
    ...
    return {
        ...
        "dX_mas": dX_mas,
        "dY_mas": dY_mas,
        "lod_total_ms": lod_total_ms,  # <-- TAMBAHAN
...
def print_report(data: Dict) -> None:
    ...
    print(" ** EPHEMERIS ENGINE : VSOP2013 + ELP/MPP02 **")
    print(" (VSOP2013: Fienga et al. 2013, IMCCE)")
    print("=" * 80)

    # [1] TIME
    print("\n[1] TIME SYSTEMS (JD, TAI, TT, TDB, UT1)")
    print("-" * 80)
    print(f"  UTC Epoch            : {ts.to_iso(calendar=cal)} (ISO 8601, {cal})")
    ...
    # [2] EOP & LOD
    print("\n[2] EARTH ORIENTATION & LENGTH OF DAY")
    print("-" * 80)
    ...
    print(f"  LOD                  : {data['lod_total_ms']:12.6f} ms")
    print(f"  Day Length (SI)      : {86400.0 + data['lod_total_ms'] / 1000.0:12.6f} s")
    ...
    print(f"    Distance (km) : {sun['dist_km']:15.3f}")
    print(f"    Distance (AU) : {sun['dist_au']:15.9f}")
    ...
    print(f"    Distance (km) : {moon['dist_km']:15.3f}")
    print(f"    Distance (AU) : {moon['dist_au']:15.9f}")

    # [8] Horizontal (with refraction)
    ...
    print(f"  Orthometric Height   : {OBSERVER_HEIGHT_M:.3f} m (XGM2019e-2159)")
    ...
    print(f"  Ephemeris Sources    : VSOP2013 (Sun), ELP/MPP02 (Moon)")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compute high-precision ephemeris for a given date/time (VSOP2013 engine).",
        epilog="Examples:\n"
               "  python AstroCalc_VSOP2013.py                        # interactive\n"
               "  python AstroCalc_VSOP2013.py 2026-06-17T15:30:00    # Gregorian (default)\n"
               "  python AstroCalc_VSOP2013.py 1041-11-06T04:00:00 --calendar julian\n"
               "  python AstroCalc_VSOP2013.py -4713-01-01T12:00:00   # negative year"
    )
    parser.add_argument(
