...
class LocalEGM2008Grids:
    ...
    def get_gravity_earth(self, lat, lon):
        """Surface gravity (g) from XGM2019e-2159 grid (mGal)."""
...
class PawitraGeophysics:
    ...
    def dynamic_gravity_anomalies(self, lat_deg, lon_deg, orthometric_height_m, 
        ...
        # ===== PERUBAHAN UTAMA: pilih sumber g_obs =====
        if use_grid_g_obs and hasattr(self.grids, 'get_gravity_earth'):
            # Ambil langsung dari grid XGM2019e-2159 (baseline statis yang lebih akurat)
            # + tambahkan OTL dinamis
            g_obs = self.grids.get_gravity_earth(lat_deg, lon_deg) + otl_gravity_correction_mgal
        else:
